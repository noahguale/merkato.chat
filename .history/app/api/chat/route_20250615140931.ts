...
export async function GET(req: NextRequest) {
	...
	try {
		...
		const messages = await convex.query(api.chat.getMessages, {
			threadId: chatId as any,
		})

		const mostRecentMessage = messages.at(-1)

		// Check if there's an active stream for this thread
		const activeStream = await convex.query(api.streams.getActiveStream, {
			threadId: chatId as any,
		})
...
export async function POST(req: NextRequest) {
	try {
		...
		if (resumeStreamId && threadId) {
			const existingStream = await convex.query(api.streams.getStreamById, {
		...
		if (threadId) {
			try {
				if (resumeStreamId) {
					await convex.mutation(api.streams.updateStreamSession, {
						streamId,
						status: 'active',
						partialContent: accumulatedContent,
					})
				} else {
					await convex.mutation(api.streams.createStreamSession, {
		...
		try {
			const result = streamText({
				...
				onFinish: async ({ response }) => {
					if (threadId) {
						try {
							await convex.mutation(api.streams.updateStreamStatus, {
								streamId,
								status: 'completed',
				...
				onError: (error) => {
					...
					if (threadId) {
						convex
							.mutation(api.streams.updateStreamStatus, {
								streamId,
								status: 'failed',
							})
							.catch(console.error)
					...
					try {
						while (true) {
							...
							if (threadId && accumulatedContent.length % 100 === 0) {
								convex
									.mutation(api.streams.updatePartialContent, {
										streamId,
										partialContent: accumulatedContent,
									})
									.catch(console.error)
						...
						// Save final content
						if (threadId) {
							await convex.mutation(api.streams.updatePartialContent, {
								streamId,
								partialContent: accumulatedContent,
							})
						}
