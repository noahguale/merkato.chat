		...
		try {
			switch (provider) {
				...
				default:
					return NextResponse.json(
						{
							error: `Unsupported provider: ${provider}. Supported providers: openai, google, anthropic, openrouter`,
						},
						{ status: 400 }
					)
			}
			...
			return NextResponse.json(
				{
					error: `Failed to initialize ${provider} provider. Please check your API key.`,
				},
				{ status: 500 }
			)
		}
